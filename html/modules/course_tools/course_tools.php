...
if ($is_adminOfCourse){
	...
	if  (isset($_REQUEST['toolStatus']) ){
     ...
     if (invalid_token()) {
      ...
      <p><a href='".htmlspecialchars($_SERVER[PHP_SELF])."'>$langAgain</a></p></td></tr></tbody></table><br /><br />";
		...
		while ($i< $loopCount) {
			if (!isset($tool_id)) {
				$tool_id = " (`id` = " . intval($tool_stat_active[$i]) .")" ; // MY CODE
			}
			else {
  ...
  // ------------------------- // MY CODE
      ...
      if($flag == false){
      	$tool_content .= "<p class=\"caution_small\">Password Failed! Try again!<br /><a href=\"".htmlspecialchars($_SERVER[PHP_SELF])."?action=1\">$langHome</a></p><br />";
  			draw($tool_content, 2, 'course_tools');
  			exit();
      }
  ...
  if ($is_adminOfCourse && @$action == 2) {//add external link // MY CODE
