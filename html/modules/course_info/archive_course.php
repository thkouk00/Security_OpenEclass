...
if($is_adminOfCourse) {
	...
	// ----------------------------------------
		...
		if ($zipCourse->create($webDir.$archiveDir, PCLZIP_OPT_REMOVE_PATH, "$webDir") == 0) {
			...
			if(isset($_POST['password'])){ // MY CODE BEGINS
...
function backup_course_units($f) {
	...
	while($q = mysql_fetch_array($res)) {
		fputs($f, "course_units(".
			...
			quote($q['visibility']).", ".
			quote($q['order']).", array(");
		$res2 = db_query("SELECT * FROM unit_resources WHERE unit_id = ".intval($q[id]), $mysqlMainDb); // MY CODE
		$begin = true;
...
function backup_course_details($f, $course) {
	global $mysqlMainDb;
		$course = xss_sql_filter($course); // MY CODE
