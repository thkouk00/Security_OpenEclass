...
if (isset($formSent)) {
	$CurrentAttempt = mysql_fetch_array(db_query("SELECT COUNT(*) FROM exercise_user_record
		WHERE eid='".intval($eid_temp)."' AND uid='".intval($uid)."'", $currentCourseID));// MY CODE
	++$CurrentAttempt[0];
	if (($exerciseAllowedAttemtps == 0) or ($CurrentAttempt[0] <= $exerciseAllowedAttemtps)) { // if it is allowed
		...
		if (($exerciseType == 1) or (($exerciseType == 2) and ($nbrQuestions == $questionNum))) { // record
			...
			/* BEGIN */
			$eid_temp = intval($eid_temp);// MY CODE
			$uid = intval($uid);// MY CODE
			$RecordStartDate = xss_sql_filter($RecordStartDate);// MY CODE
			$RecordEndDate = xss_sql_filter($RecordEndDate);// MY CODE
			/* END */
