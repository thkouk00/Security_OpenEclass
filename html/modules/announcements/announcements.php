...
hContent;
    $cours_id = intval($cours_id); // MY CODE
    ...
    if (isset($thisAnnouncementId) && $thisAnnouncementId && isset($sortDirection) && $sortDirection) {
        ...
        while (list ($announcementId, $announcementOrder) = mysql_fetch_row($result)) {
            if (isset ($thisAnnouncementOrderFound) && $thisAnnouncementOrderFound == true) {
                ...
                $nextAnnouncementOrder = $announcementOrder; // MY CODE
                $nextAnnouncementId = intval($nextAnnouncementId); // MY CODE
                $nextAnnouncementOrder = intval($nextAnnouncementOrder);
    ...
    if (isset($_GET['modify'])) {
	    ...
	    $contentToModify = htmlspecialchars($myrow['contenu']);
	...
	--------------------------------------*/
   // MY CODE
    $antitle = xss_sql_filter($_POST['antitle']);
    $newContent = xss_sql_filter($_POST['newContent']);
    $newContent = $_POST['newContent'];
    if (isset($_POST['submitAnnouncement'])) {
        // modify announcement
        if ($id) {
          $id = intval($_POST['id']);  // MY CODE : 217-219
          db_query("UPDATE annonces SET contenu = ".justQuote($newContent).",
    title = ".justQuote($antitle).", temps = NOW()
    WHERE id = $id", $mysqlMainDb);
        ...
        // add new announcement
            ...
            $order = $orderMax + 1;
            $order = intval($order); // MY CODE 233-235
            // INSERT ANNOUNCEMENT    // MY CODE:
            db_query("INSERT INTO annonces SET contenu =". justQuote($newContent).",
  title = ".justQuote($antitle).", temps = NOW(),
  cours_id = $cours_id, ordre = $order");
        ...
        if (isset($_POST['emailOption']) and $_POST['emailOption']) {
            $emailContent = xss_sql_filter($_POST['antitle']) . // MY CODE
                            "<br><br>" .
                            xss_sql_filter($_POST['newContent']); // MY CODE
