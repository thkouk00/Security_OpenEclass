...
//	$tool_content .= "<table width=\"99%\"><tbody><tr><td>";
	$code = xss_sql_filter($_POST['cc']); // MY CODE
	$sql=db_query("SELECT * FROM cours_user WHERE cours_id = (SELECT cours_id FROM cours WHERE code = "  . justQuote($code) . ")"); // MY CODE

	while ($m = mysql_fetch_array($sql)) {

		$sql1 = db_query("SELECT email FROM user WHERE user_id='".intval($m["user_id"])."'"); // MY CODE
