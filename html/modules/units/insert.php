...
function insert_docs($id)
{
	...
	foreach ($_POST['document'] as $file_id) {
		...
		db_query("INSERT INTO unit_resources SET unit_id=$id, type='doc', title=" .
			 autoquote(xss_sql_filter($title)) . ", comments=" . autoquote(xss_sql_filter($file['comment'])) .
...
function insert_text($id)
{
	...
	db_query("INSERT INTO unit_resources SET unit_id=$id, type='text', title='', 
		comments=" . autoquote(xss_sql_filter($comments)) . ", visibility='v', `order`=$order, `date`=NOW(), res_id=0",
		$GLOBALS['mysqlMainDb']);
...
function insert_lp($id)
{
	...
	foreach ($_POST['lp'] as $lp_id) {
		...
		if ($lp['visibility'] == 'HIDE') {
			...
			quote(xss_sql_filter($lp['name'])) . ", comments=" . quote(xss_sql_filter($lp['comment'])) .
		...
		$row = mysql_fetch_array(db_query("SELECT * FROM $table
			WHERE id = $res_id", $GLOBALS['currentCourseID']), MYSQL_ASSOC);
				...
				// BEGIN
		      	...
		      	$res_id = intval($res_id);
		      	// END
                db_query("INSERT INTO unit_resources SET unit_id=$id, type='$table', title=" . quote(xss_sql_filter($row['titre'])) . ", comments=" . quote(xss_sql_filter($row['description'])) . ", visibility='v', `order`=$order, `date`=NOW(), res_id=$res_id", $GLOBALS['mysqlMainDb']);
	}
	header('Location: index.php?id=' . $id);
	exit;
}
...
function insert_work($id)
{
	...
	foreach ($_POST['work'] as $work_id) {
		...
		db_query("INSERT INTO unit_resources SET
                                unit_id = $id,
                                type = 'work',
                                title = " . quote(xss_sql_filter($work['title'])) . ",
                                comments = " . quote(xss_sql_filter($work['description'])) . ",
...
function insert_exercise($id)
{
	...
	foreach ($_POST['exercise'] as $exercise_id) {
		...
		db_query("INSERT INTO unit_resources SET unit_id=$id, type='exercise', title=" .
			quote(xss_sql_filter($exercise['titre'])) . ", comments=" . quote(xss_sql_filter($exercise['description'])) .
	...
	foreach ($_POST['forum'] as $for_id) {
		...
		if (count($ids) == 2) {
			...
			db_query("INSERT INTO unit_resources SET unit_id=$id, type='topic', title=" .
				quote(xss_sql_filter($topic['topic_title'])) .", visibility='v', `order`=$order, `date`=NOW(), res_id=$topic[topic_id]",
			$GLOBALS['mysqlMainDb']);		
		} else {
                        $forum_id = $ids[0];
			...
			db_query("INSERT INTO unit_resources SET unit_id=$id, type='forum', title=" .
				quote(xss_sql_filter($forum['forum_name'])) . ", comments=" . quote(xss_sql_filter($forum['forum_desc'])) .
...
function insert_wiki($id)
{
	...
	foreach ($_POST['wiki'] as $wiki_id) {
		...
		db_query("INSERT INTO unit_resources SET unit_id=$id, type='wiki', title=" .
			quote(xss_sql_filter($wiki['title'])) . ", comments=" . quote(xss_sql_filter($wiki['description'])) .
