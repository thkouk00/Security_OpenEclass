...
if($is_adminOfCourse) {
	...
	$sql="SELECT DISTINCT uid FROM `exercise_user_record` WHERE eid='".xss_sql_filter($_GET['exerciseId'])."'"; // MY CODE
	$result = mysql_query($sql);
	while($row=mysql_fetch_array($result)) {
		$sid = intval($row['uid']); // MY CODE
		$StudentName = db_query("select nom, prenom from user where user_id='$sid'", $mysqlMainDb);
		$theStudent = mysql_fetch_array($StudentName);
		$nom = xss_sql_filter($theStudent["nom"]); // MY CODE
		$prenom = xss_sql_filter($theStudent["prenom"]);	 // MY CODE
		mysql_select_db($currentCourseID);
		$sql2="SELECT DATE_FORMAT(RecordStartDate, '%Y-%m-%d / %H:%i') AS RecordStartDate,
			...
			FROM `exercise_user_record` WHERE uid='$sid' AND eid='".xss_sql_filter($_GET['exerciseId'])."'"; // MY CODE
