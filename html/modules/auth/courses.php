	...
	if (!$fac) {
		...
		if (isset($result))  {
			...
			while ($fac = mysql_fetch_array($result)) {
				...
				$n=db_query("SELECT COUNT(*) FROM cours_faculte WHERE facid='".intval($fac[id])."'"); // MY CODE
...
function expanded_faculte($fac_name, $facid, $uid) {
	...
	$retString = "";
  $facid = intval($facid); // MY CODE
  $uid = intval($uid);  // MY CODE
  $fac_name = xss_sql_filter($fac_name); // MY CODE
...
function collapsed_facultes_vert($fc) {
	...
	while ($fac = mysql_fetch_array($result)) {
		...
		$n = db_query("SELECT COUNT(*) FROM cours
			WHERE cours.faculte='".xss_sql_filter($fac[f])."' AND cours.visible <> '0'"); // MY CODE
                $r = mysql_fetch_array($n);
...
function collapsed_facultes_horiz($fc) {
	...
	$retString = "\n   <form name='depform' action='".htmlentities($_SERVER[PHP_SELF])."' method='get'>\n";
