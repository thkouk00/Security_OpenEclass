...
if ($is_adminOfCourse) {
	...
	if (isset($mkInvisibl) or isset($mkVisibl)) {
		if (@$mkInvisibl == true) {
			...
			// Add all data to the main table. /* MY CODE BEGINS */
	...
	if (isset($_POST['submit'])) {
		...
		$result = db_query($sql, $currentCourseID);
			...
			if (substr_count($sql,"INSERT") == 1) {
				...
				(lesson_event_id, titre, contenu, day, hour, lasting, lesson_code) /* MY CODE BEGINS */
				VALUES('".xss_sql_filter($perso_query_result[0])."','".xss_sql_filter($perso_query_result[1])."',
				'".xss_sql_filter($perso_query_result[2])."','".xss_sql_filter($perso_query_result[3])."',
				'".xss_sql_filter($perso_query_result[4])."','".xss_sql_filter($perso_query_result[5])."',
				'".$currentCourseID."')"; /* ENDS */
			}
...
';
	...
	if (isset($id) && $id) {
		...
		$id = $myrow["id"];
		$titre = $myrow["titre"];
		$contenu= htmlspecialchars($myrow["contenu"]);
    ...
    <tr>
      ...
      ";
		...
		$tool_content .= "<tr><th class='left'>$langDetail:</th>
			<td><textarea id='xinha' name='contenu'>".strip_tags($contenu,'<br><a><b><h3><h4><h5><h6><u><del><li><span><p><ol><ul><hr><table><tbody><body><head><tr><td><strong>')."</textarea></td>
			</tr>
			<tr><th class='left'>&nbsp;</th>
