...
function announcement($text, $date, $order, $title = '') {
	...
	$new_course_id = intval($new_course_id); // MY CODE
	if (!$action) return;
...
$login = xss_sql_filter($login);
	...
	VALUES ($new_course_id, ".intval($userid_map[$userid]).", $statut)"); // MY CODE
