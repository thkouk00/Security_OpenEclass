...
function isCourseMember($id)
{
    global $dropbox_cnf, $dropbox_lang, $mysqlMainDb;
        $id = intval($id); // MY CODE
            $dropbox_cnf["cid"] = xss_sql_filter($dropbox_cnf["cid"]); // MY CODE
...
function removeUnusedFiles()
{
	...
	$sql = "DELETE FROM `" . $dropbox_cnf["postTbl"] . "` WHERE fileId='" . intval($res['id']) . "'"; // MY CODE
        $result1 = db_query($sql, $currentCourseID);
        $sql = "DELETE FROM `" . $dropbox_cnf["fileTbl"] . "` WHERE id='" . intval($res['id']) . "'"; // MY CODE
        $result1 = db_query($sql, $currentCourseID);
...
function removeMoreIfMailing($fileId)
{
    ...
    $sql = "SELECT p.recipientId FROM `" . $dropbox_cnf["postTbl"] . "` p
		WHERE p.fileId = '" . intval($fileId) . "'";// MY CODE
    $result = db_query($sql, $currentCourseID);
    ...
    if ($res = mysql_fetch_array($result))
    {
	    ...
	    {
	        $sql = "DELETE FROM `" . $dropbox_cnf["personTbl"] . "` WHERE personId='" . intval($mailingPseudoId) . "'"; // MY CODE
	        $result1 = db_query($sql, $currentCourseID);
        }
    }
}
