		...
		*/
		foreach ($this->recipients as $rec) {
			$sql="INSERT INTO `".$dropbox_cnf["postTbl"]."`
				(fileId, recipientId)
				VALUES ('".addslashes(intval($this->id))."', '".addslashes(intval($rec["id"]))."')"; // MY CODE
...
class Dropbox_Person {
	...
	function Dropbox_Person ($userId, $isCourseAdmin, $isCourseTutor) {
		...
		* Fill in properties
		*/
		$this->userId = intval($userId); // MY CODE
		...
		*/
		...
		foreach ($this->receivedWork as $w) {
			db_query("DELETE FROM `".$dropbox_cnf["personTbl"]."`
				WHERE personId='".intval($this->userId)."' AND fileId='".intval($w->id)."'", $currentCourseID); // MY CODE
		}
	...
	function deleteReceivedWork ($id) {
		...
		*/
		...
		//delete entries in person table concerning received works
		db_query("DELETE FROM `".$dropbox_cnf["personTbl"]."`
			WHERE personId='".intval($this->userId)."' AND fileId='".intval($id)."'", $currentCourseID); // MY CODE
