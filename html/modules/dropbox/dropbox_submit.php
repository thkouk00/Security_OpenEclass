...
if (!isset( $_POST['authors']) || !isset( $_POST['description']))
	...
	if (preg_match('/\.(ade|adp|bas|bat|chm|cmd|com|cpl|crt|exe|hlp|hta||sh|php|html|png|jpg|' .'inf|ins|isp|jse|lnk|mdb|mde|msc|msi|msp|mst|pcd|pif|reg|scr|sct|shs|' .'shb|url|vbe|vbs|wsc|wsf|wsh)$/', $_FILES['file']['name'])) {
		$error = TRUE;
		$errormsg = "Not acceptable file type. Please add zip file";	
	}
	...
	if (!preg_match($dropbox_cnf["mailingZipRegexp"], $mailing_title, $nameParts))
	{
		...
		if (!$error)
		{
			...
			if ( isset( $_GET['mailingSend']))
			{
				...
				// set filesize to zero on send, to avoid 2nd send (see index.php) 						// MY CODE  below
